...
private let globalInstance = SnackBoard()

/**
 The `SnackBoard` class provides the interface for showing and hiding messages.
 ...
 */
open class SnackBoard {
    
    
    @objc public  enum SnackBoardDuration: Int {
        case short = 1
        case middle = 3
        case long = 5
    ...
    public enum PresentationContext {
        ...
        */
        ...
        /**
         ...
         SnackBoard automatically increases the top margins of any message
        ...
        */
        ...
        /**
         Displays the message in a new window, at the specified window level,
         in the specified window scene. SnackBoard automatically increases the top margins
    ...
    public enum DimMode {
        ...
        */
        ...
        /**
         ...
         SnackBoard does not apply alpha transparency to the color, so any alpha
    ...
    */
    ...
    public struct Config {
        ...
        */
        ...
        /**
         The user interface style to use when SnackBoard displays a message its own window.
        ...
        */
        ...
        /**
         If specified, SnackBoard calls this closure when an instance of
         `WindowViewController` is needed. Use this if you need to supply a custom subclass
         of `WindowViewController`.
         */
        public var windowViewController: ((_ config: SnackBoard.Config) -> WindowViewController)?
    ...
    fileprivate let messageQueue = DispatchQueue(label: "it.swiftkick.SnackBoard", attributes: [])
...
extension SnackBoard: PresenterDelegate {
...
/**
 ...
 SnackBoard provides several default nib files in the Resources folder that can be
...
extension SnackBoard {
    ...
    /**
     ...
     first followed by the SnackBoard bundle.
    ...
    /**
     ...
     with matching type `T`. The main bundle is searched first followed by the SnackBoard bundle.
    ...
    fileprivate class func internalViewFromNib<T: UIView>(named name: String, bundle: Bundle? = nil, filesOwner: AnyObject = NSNull.init()) throws -> T {
        ...
        #if swift(>=4.1)
        guard let view = arrayOfViews.compactMap( { $0 as? T} ).first else { throw SnackBoardError.cannotLoadViewFromNib(nibName: name) }
        #else
        guard let view = arrayOfViews.flatMap( { $0 as? T} ).first else { throw SnackBoardError.cannotLoadViewFromNib(nibName: name) }
        #endif
        return view
    }
}
...
/*
 ...
 This extension provides a shared instance of `SnackBoard` and a static API wrapper around
 this instance for simplified syntax. For example, `SnackBoard.show()` is equivalent
 to `SnackBoard.sharedInstance.show()`.
 */

extension SnackBoard {
    
    /**
     A default shared instance of `SnackBoard`. The `SnackBoard` class provides
     a set of static APIs that wrap calls to this instance. For example, `SnackBoard.show()`
     is equivalent to `SnackBoard.sharedInstance.show()`.
     */
    public static var sharedInstance: SnackBoard {
    ...
    public static func display(message: String, mode: Theme, duration: SnackBoardDuration) {
        let view = MessageView.viewFromNib(layout: .tabView)
        view.configureContent(title: message, body: "" , iconImage: nil, iconText: nil, buttonImage: nil, buttonTitle: "", buttonTapHandler: { _ in SnackBoard.hide() })
        view.configureTheme(mode, iconStyle: .default)
        var config = SnackBoard.defaultConfig
        config.duration = .seconds(seconds: TimeInterval(duration.rawValue))
        SnackBoard.show(config: config, view: view)
    }
    
    public static func show(title: String, message: String, mode: Theme, duration: SnackBoardDuration) {
        let view = MessageView.viewFromNib(layout: .tabView)
        view.configureContent(title: title, body: message , iconImage: nil, iconText: nil, buttonImage: nil, buttonTitle: "", buttonTapHandler: { _ in SnackBoard.hide() })
        view.configureTheme(mode, iconStyle: .default)
        var config = SnackBoard.defaultConfig
