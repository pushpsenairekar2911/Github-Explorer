...
/*
 ...
 copied the nib files because SnackBoard looks for them in the main
 bundle first. See `SnackBoard` for additional nib loading options.
 */
...
extension MessageView {
    ...
    public static func viewFromNib<T: MessageView>(layout: Layout, filesOwner: AnyObject = NSNull.init()) -> T {
        return try! SnackBoard.viewFromNib(named: layout.rawValue)
    }
    ...
    public static func viewFromNib<T: MessageView>(layout: Layout, bundle: Bundle, filesOwner: AnyObject = NSNull.init()) -> T {
        return try! SnackBoard.viewFromNib(named: layout.rawValue, bundle: bundle, filesOwner: filesOwner)
    }
}
...
/*
 ...
 SnackBoard does not try to be clever by adjusting the layout based on 
