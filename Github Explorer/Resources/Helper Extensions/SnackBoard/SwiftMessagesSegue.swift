...
/**
 `SnackBoardSegue` is a configurable subclass of `UIStoryboardSegue` that utilizes
 SnackBoard to present and dismiss modal view controllers. It performs these transitions by
 becoming your view controller's `transitioningDelegate` and calling SwiftMessage's `show()`
 and `hide()` under the hood.

 To use `SnackBoardSegue` with Interface Builder, control-drag a segue, then select
 "swift messages" from the Segue Type dialog. This configures a default transition. There are
 two suggested ways to further configure the transition by setting options on `SnackBoardSegue`.
 First, and recommended, you may subclass `SnackBoardSegue` and override `init(identifier:source:destination:)`.
 ...
 presenting view controller and downcast the segue to `SnackBoardSegue`.

 `SnackBoardSegue` can be used without an associated storyboard or segue by doing the following in
 the presenting view controller.

     let destinationVC = ... // make a reference to a destination view controller
     let segue = SnackBoardSegue(identifier: nil, source: self, destination: destinationVC)
 ...
 However, these parameters can be customized by initializing the view controller with a `SnackBoard.Config` that has the `SnackBoard.Config.presentationContext` set to either `.window` or `.windowScene`:

 + note: Some additional details:
 1. Your view controller's view will be embedded in a `SnackBoard.BaseView` in order to
 utilize some SnackBoard features. This view can be accessed and configured via the
 `SnackBoardSegue.messageView` property. For example, you may configure a default drop
 shadow by calling `segue.messageView.configureDropShadow()`.
 2. SnackBoardSegue provides static default view controller sizing based on device.
...
open class SnackBoardSegue: UIStoryboardSegue {
    ...
    /// The presentation style to use. See the SnackBoard.PresentationStyle for details.
    public var presentationStyle: SnackBoard.PresentationStyle {
        get { return messenger.defaultConfig.presentationStyle }
        set { messenger.defaultConfig.presentationStyle = newValue }
    }

    /// The dim mode to use. See the SnackBoard.DimMode for details.
    public var dimMode: SnackBoard.DimMode {
        get { return messenger.defaultConfig.dimMode}
        set { messenger.defaultConfig.dimMode = newValue }
    }
    
    // duration
    public var duration: SnackBoard.Duration {
    ...
    public var eventListeners: [SnackBoard.EventListener] {
    ...
    */
    ...
    /**
     The view that is passed to `SnackBoard.show(config:view:)` during presentation.
     The view controller's view is installed into `containerView`, which is itself installed
     into `messageView`. `SnackBoardSegue` does this installation automatically based on the
    ...
    */
    ...
    private var messenger = SnackBoard()
    private var selfRetainer: SnackBoardSegue? = nil
...
extension SnackBoardSegue: UIViewControllerTransitioningDelegate {
        ...
        messenger.defaultConfig.eventListeners.append { [unowned self] in
            ...
            case .didHide:
                if let completeTransition = self.hider.completeTransition {
                    ...
                    // Case where message is internally hidden by SnackBoard, such as with a
...
extension SnackBoardSegue {
    private class TransitioningPresenter: NSObject, UIViewControllerAnimatedTransitioning {
        ...
        private weak var segue: SnackBoardSegue?

        fileprivate init(segue: SnackBoardSegue) {
            self.segue = segue
        }
